function [ bool ] = Curve_Fitting_Model3( class_obj )
%  model3 : y=a*e^(-b*(x-min_SNR)/(max_SNR-min_SNR))+c   arg: a b c
   % F = @(a,x) a(1).*exp(-a(2)*x) + a(3);
   path_txt = [class_obj.m_path,'Model3_模型参数'];
if ~exist(path_txt)
   mkdir(path_txt);
else
   rmdir(path_txt,'s');
...
path_txt = [path_txt,'\\Model3_模型参数.txt']; 
   
Curve_Fitting_Model3_P(class_obj,path_txt);
%Curve_Fitting_Model3_L(class_obj);
    
end
function [ bool ] = Curve_Fitting_Model3_P(class_obj,path_txt)
    path = [class_obj.m_path,'Model3_伪距残差-信噪比_曲线拟合'];
    if ~exist(path)
        mkdir(path);
    else
        rmdir(path,'s');
...
%             end
            ...
            class_obj.m_Pall_CN0_model3(1,sys,f) = X0(1);
            class_obj.m_Pall_CN0_model3(2,sys,f) = X0(2);
            class_obj.m_Pall_CN0_model3(3,sys,f) = X0(3);
            class_obj.m_Pall_CN0_model3(4,sys,f) = resnorm;
            class_obj.m_Pall_CN0_model3(5,sys,f) = X(1);
            %绘图
    ...
    [~,~,f1]=size(class_obj.m_Pall_CN0_model3); 
    if f1 == 1 
       ADD = zeros(5,5,1);
       class_obj.m_Pall_CN0_model3 = cat(3,class_obj.m_Pall_CN0_model3,ADD);
    end
    fp = fopen(path_txt,'a');
    fprintf(fp,'频率1\r\n');
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,1,1));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,2,1));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,3,1));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,4,1));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,5,1));
    
    fprintf(fp,'频率2\r\n');
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,1,2));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,2,2));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,3,2));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,4,2));
    fprintf(fp,'%.4f %.4f %.4f %.4f %.1f\r\n',class_obj.m_Pall_CN0_model3(:,5,2));
...
function [ bool ] = Curve_Fitting_Model3_L(class_obj)
    path = [class_obj.m_path,'Model3_相位残差-信噪比_曲线拟合'];
    if ~exist(path)
        mkdir(path);
    else
        rmdir(path,'s');
...
%             end
            ...
            class_obj.m_Lall_CN0_model3(1,sys,f) = X0(1);
            class_obj.m_Lall_CN0_model3(2,sys,f) = X0(2);
            class_obj.m_Lall_CN0_model3(3,sys,f) = X0(3);
            class_obj.m_Lall_CN0_model3(4,sys,f) = resnorm;
            class_obj.m_Lall_CN0_model3(5,sys,f) = X(1);
            %绘图
            plot_x1 = X(1):0.01:X(length(X));
            plot_y1 = X0(1)*exp(-X0(2)*(plot_x1-min_SNR)/(max_SNR-min_SNR))+X0(3);
